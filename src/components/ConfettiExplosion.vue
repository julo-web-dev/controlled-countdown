<!-- Copied from https://github.com/valgeirb/vue-confetti-explosion/blob/main/src/ConfettiExplosion.vue (MIT license) -->
...
const {
  particleCount = 150,
  particleSize = 12, // max height for particle rectangles, diameter for particle circles
  duration = 3500,
  colors = ['#FFC700', '#FF0000', '#2E3191', '#41BBC7'],
  force = 0.5, // 0-1 roughly the vertical force at which particles initially explode,
  stageHeight = 800, // pixels the particles will fall from initial explosion point
  stageWidth = 1600, // horizontal spread of particles in pixels
...
const particles = computed(() => createParticles(particleCount, colors));
...
const isValid = computed(() =>
  validate(particleCount, duration, colors, particleSize, force, stageHeight, stageWidth),
);
...
<template>
  ...
  >
    <div
      ...
      :key="degree"
      class="particle"
      :ref="(el) => el && setItemRef(el as HTMLDivElement, degree)"
    >
